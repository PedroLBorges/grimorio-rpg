            ...
            <div class="rounded-2xl border-4 border-amber-900/60 bg-amber-100/90 p-6 text-amber-950 shadow-2xl sm:p-8 [&_label]:font-serif [&_label]:font-bold [&_input]:border-amber-800/40 [&_input]:bg-amber-50/70 [&_input]:focus:border-purple-800 [&_input]:focus:ring-purple-800 [&_select]:border-amber-800/40 [&_select]:bg-amber-50/70 [&_textarea]:border-amber-800/40 [&_textarea]:bg-amber-50/70">
                <form method="POST" action="{{ route('characters.weapons.update', [$character, $weapon]) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $weapon->name) }}"
                    ...
                    <div>
                        ...
                        <select name="ability" class="w-full rounded border-gray-300" required>
                            <option value="strength" {{ old('ability', $weapon->ability) === 'strength' ? 'selected' : '' }}>
                                Força
                            </option>
                            <option value="dexterity" {{ old('ability', $weapon->ability) === 'dexterity' ? 'selected' : '' }}>
                                Destreza
                            </option>
                        </select>
                    </div>
                    ...
                    <div>
                        <label class="flex items-center gap-3 rounded border p-3">
                            <input type="checkbox" name="proficient" value="1" {{ old('proficient', $weapon->proficient) ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="damage_dice" value="{{ old('damage_dice', $weapon->damage_dice) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="damage_type" value="{{ old('damage_type', $weapon->damage_type) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="range" value="{{ old('range', $weapon->range) }}"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                                  class="w-full rounded border-gray-300"
                                  placeholder="Ex: leve, versátil, duas mãos, arremesso...">{{ old('description', $weapon->description) }}</textarea>
                    </div>
